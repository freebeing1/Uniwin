...
class ImageNetPairedDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.mean = opt['mean'] if 'mean' in opt else None # mean=[0.485, 0.456, 0.406]
        self.std = opt['std'] if 'std' in opt else None # std=[0.229, 0.224, 0.225]
    ...
    def __getitem__(self, index):
        ...
        size_h = max(size_h, self.opt['H_size'])
        size_w = max(size_w, self.opt['H_size'])
        ...
        return {'L': img_lq, 'H': img_gt, 'H_path': gt_path}
