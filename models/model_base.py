...
class ModelBase():
    ...
    def load_network(self, load_path, network, strict=True, param_key='params'):
        ...
        if strict:
            ...
            if self.opt['scale'] == 2 or self.opt['scale'] == 4:
                for (key_old, param_old) in state_dict_old.items():
                    state_dict[key_old] = param_old
            elif self.opt['scale'] == 3:
                for key_old, param_old in state_dict_old.items():
                    if key_old == 'upsample.0.weight' or key_old == 'upsample.0.bias':
                        continue
                    state_dict[key_old] = param_old
            else:
                raise NotImplementedError(f'SR with scale [{self.opt["scale"]}] is not implemented.')
