...
from basicsr.data.paired_image_dataset import PairedImageDataset as define_Dataset
from models.model_sr import ModelSR as define_Model

def main(json_path='options/swinir_lmdb.json'):
    '''
    # ----------------------------------------
    # Step--1 (prepare opt)
    # ----------------------------------------
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0:
                ...
                for test_data in test_loader:
                    ...
                    SR_save_img_path = os.path.join(
                        img_dir, f'{img_name}_{current_step}.png')
                    util.imsave(SR_img, SR_save_img_path)

                    # -----------------------
                    # calculate PSNR
                    # -----------------------
                    current_psnr = util_calculate.calculate_psnr(SR_img, H_img, crop_border=border)
                    current_psnr_y = util_calculate.calculate_psnr(cv2.cvtColor(SR_img, cv2.COLOR_BGR2RGB), cv2.cvtColor(H_img, cv2.COLOR_BGR2RGB), crop_border=border, test_y_channel=True)
                    logger.info(
                        '{:->4d}--> {:>10s} | {:<4.2f}dB | (PSNR_Y {:<4.2f}dB)'.format(idx, image_name_ext, current_psnr, current_psnr_y))
                ...
                # testing log
                logger.info('<epoch:{:3d}, iter:{:8,d}, Average PSNR : {:<.2f}dB, Average PSNR_Y : {:<.2f}dB'.format(
